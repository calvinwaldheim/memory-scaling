...
def _soft_forget_row(
    ...
    return (row_id, rule, context, memory_type, domain, scope, quality_score,
            forgotten_at, forgotten_reason, forgotten_by_user)


def test_soft_forget_marks_row(monkeypatch) -> None:
    conn = FakeConnection(script=[_soft_forget_row()])
    monkeypatch.setattr(storage, "_get_connection", lambda: conn)
    ...
    assert conn.commits == 1
    # Only one statement should have executed when the UPDATE landed.
    assert len(conn.cursor_obj.executed) == 1
...
def _locked_row(
    ...
    return (row_id, project_id, project_type, memory_type, scope, user_id,
            domain, rule, superseded_at, superseded_by, forgotten_at)
...
def test_supersede_happy_path(monkeypatch) -> None:
    ...
    # Three SQL statements executed inside the single transaction.
